...
# Keep the complete hook, including both SQLite integrity checks, inside the
# pod's 120-second grace period. The extra six seconds are reserved for
# Kubernetes to deliver SIGTERM to Jupyter after this hook returns.
...
def emit_alert(code: str) -> None:
    print(
        f"CW_ALERT v=1 severity=error code={code} component=runtime phase=prestop",
        file=sys.stderr,
        flush=True,
    )


def require_time_remaining(deadline: float) -> float:
...
def identify_candidate(pid: int, expected_uid: int) -> Candidate | None:
    try:
...
def sqlite_quick_check(descriptor: int, deadline: float) -> None:
    ...
    remaining = require_time_remaining(deadline)
    ...
    require_time_remaining(deadline)
    if findings != ["ok"]:
        raise PreStopFailure("prestop_sqlite_quick_check_failed")
...
def verify_shutdown_storage(
    ...
    try:
        ...
        try:
            sqlite_quick_check(checkpoint_descriptor, deadline)
        finally:
    ...
    try:
        ...
        try:
            sqlite_quick_check(state_descriptor, deadline)
        finally:
...
def run_prestop(deadline: float) -> int:
    ...
    candidate = identify_candidate(raw_matches[0], expected_uid)
    ...
    try:
        ...
        verified = identify_candidate(candidate.pid, expected_uid)
        ...
        wait_seconds = min(SHUTDOWN_WAIT_SECONDS, require_time_remaining(deadline))
    ...
    quiet_deadline = min(
        time.monotonic() + REPLACEMENT_QUIET_SECONDS,
        deadline,
    )
    ...
    print(
        ...
        f"checkpoint={checkpoint} sqlite_quick_check=ok",
        flush=True,
    )
    return 0
...
def main() -> int:
    ...
    deadline = time.monotonic() + MAX_HOOK_SECONDS
